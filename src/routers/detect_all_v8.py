...
from src.routers.cache_management import redis_client
...
# Hàm kiểm tra và chuyển đổi dữ liệu từ cache
def convert_cache_data(data):
    if isinstance(data, bytes):
        try:
            # Nếu dữ liệu là kiểu bytes, thử giải mã thành chuỗi
            data_str = data.decode('utf-8')
            
            # Kiểm tra nếu chuỗi có thể là JSON hợp lệ
            try:
                return json.loads(data_str)  # Chuyển đổi JSON thành dict
            except json.JSONDecodeError:
                # Nếu không phải JSON, thử chuyển thành Python object bằng literal_eval
                try:
                    return ast.literal_eval(data_str)  # Chuyển chuỗi thành dict hoặc list
                except (ValueError, SyntaxError):
                    return data_str  # Trả lại chuỗi nếu không thể phân tích
        except UnicodeDecodeError:
            return data  # Nếu không thể giải mã thành chuỗi, trả lại dữ liệu gốc
    return data  # Nếu không phải bytes, trả lại dữ liệu gốc


def parseToJson(data_string):
    try:
        # Thay thế dấu nháy đơn thành dấu nháy kép để chuỗi trở thành JSON hợp lệ
...
# Hàm xử lý bất đồng bộ, không cần dùng asyncio.run() nữa
async def process_location(user_input, collection_data):
    ...
    # cache_key = hashlib.md5(formatted_input.encode('utf-8')).hexdigest()
    cache_key = formatted_input
    cached_result = await redis_client.get(cache_key)
    if cached_result:
        # Trả về dữ liệu cached mà không cần decode
        return {'data': parseToJson(cached_result)}

    # Nếu không có trong cache, tìm kiếm kết quả tốt nhất
    ...
    # await redis_client.set(cache_key, str(result['data']))
    # await redis_client.ltrim('cache_list', 0, CACHE_MAX_SIZE - 1)  # Giới hạn số lượng key

    return result


# Hàm xử lý đồng bộ (sử dụng trực tiếp await thay vì asyncio.run())
def process_location_sync(user_input, collection_data):
    # Đảm bảo là sử dụng asyncio trực tiếp, không cần asyncio.run()
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(process_location(user_input, collection_data))
...
async def analyze_multiple_locations(request: AddressRequest):
    ...
    # Tạo một danh sách các futures (tác vụ bất đồng bộ) cho mỗi địa chỉ, đi kèm với index
    futures = [
        process_location(user_input, await get_data_from_collection(user_input, collection))
        for index, user_input in enumerate(request.locations)
    ]

    # Thu thập kết quả từ tất cả các task bằng asyncio.gather
    raw_results = await asyncio.gather(*futures)

    # Đánh chỉ mục vào kết quả ngay từ đầu khi thu thập kết quả
    indexed_results = [
