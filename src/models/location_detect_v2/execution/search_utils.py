...
async def extract_address(user_input, result): 
    ...
    if result['ward_score'] == 1 and result['city_score'] == 1 :
        ...
        pattern = rf"(.*?)\s*{re.escape(nomalize_vn(result['ward_name']))}"
    ...
    if result['district_score'] == 1 and result['city_score'] == 1 and result['ward_score'] != 1:
        ...
        pattern = rf"(.*?)\s*{re.escape(nomalize_vn(result['district_name']))}"
        match = re.search(pattern, user_input_tmp, re.IGNORECASE)  
        if match:
            # Lấy vị trí của delimiter
            delimiter_start = match.end(1)  
    ...
    if result['city_score'] == 1 and result['district_score'] != 1 and result['ward_score'] != 1:
        ...
        pattern = rf"(.*?)\s*{re.escape(nomalize_vn(result['city_name']))}"
        match = re.search(pattern, user_input_tmp, re.IGNORECASE)  
        if match:
            # Lấy vị trí của city
